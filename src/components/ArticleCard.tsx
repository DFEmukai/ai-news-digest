import type { ArtStyle } from '@/lib/art';
...
export default function ArticleCard({
  article,
  /** アイキャッチの構図。セクション単位で assignStyles が決める */
  artStyle,
}: {
  article: Article;
  artStyle?: ArtStyle;
  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-[6px] border border-border bg-surface shadow-[0_1px_3px_rgba(0,0,0,0.08)] transition-shadow duration-150 hover:shadow-[0_4px_12px_rgba(0,0,0,0.12)]">
      {/* アイキャッチ。記事IDをシードにした抽象画で、外部画像は使わない */}
      <div className="h-24 w-full overflow-hidden border-b border-border sm:h-28">
        <Eyecatch
          seed={article.id}
          category={article.category}
          intensity={(article.importance - 1) / 4}
          style={artStyle}
