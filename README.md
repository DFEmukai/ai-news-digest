...
各カードとページ上部に抽象画を出しているが、**画像ファイルは1枚も存在しない。**
`src/lib/art.ts` が記事IDをシードにインラインSVGを組み立てている。

外部画像を使わなかった理由:

| 案 | 却下した理由 |
|---|---|
| 元記事の og:image を表示 | 他社素材の再配布になる。「本文を保存しない」方針と矛盾する |
| 生成画像をリポジトリに置く | 1日30枚 × 2回で肥大化する。DBなし・JSONだけという前提が崩れる |
| 外部CDNに置く | オフラインで壊れ、GitHub Pages 以外への依存が増える |

**仕組み**

- 記事ID → FNV-1a でシード → mulberry32（決定的PRNG）→ 形状の座標を決める
- 構図は6種（`arcs` / `bauhaus` / `flow` / `strata` / `orbits` / `lattice`）
- 配色は**記事のカテゴリ**で決まる。色そのものは `globals.css` の
  `[data-art='...']` に CSS 変数として持たせているので、**ダークモードに自動追従する**
- 濃さは記事の `importance` に連動（重要な記事ほど絵がはっきりする）
- ページ上部のヒーローは日付がシード。日が変われば絵も変わる
- 装飾なので `aria-hidden="true"`。印刷時は非表示

**同じカテゴリのカードは配色が同じ**なので、構図まで被ると並べたときに見分けがつかない。
`assignStyles()` がセクション単位で構図を割り当て、直前と同じになるものだけ次の構図へずらす
（実測: 全セクションで隣接重複0、大半のカードはID由来の構図のまま）。

**決定性の実測**: 連続2回ビルドして31枚のSVGを抽出・比較したところ**バイト単位で一致**した。
HTML全体のハッシュは変わるが、差分は Next.js が毎回生成する buildId のみで絵とは無関係。

**コスト**: トップページは 124KB → 195KB（gzip 27KB → 39KB）。外部リクエストは0のまま。

構図を足したい場合は `ART_STYLES` に名前を追加し、`createArtwork()` の switch に
1ケース書く。色を変えたい場合は `globals.css` の `[data-art]` ブロックだけ触ればよい。
...
**1点だけ意図的に外している**: DESIGN.md は「グラデーション背景を使わない」としているが、
アイキャッチの内側でだけグラデーションを使っている。ページ背景・カード・テーブルは
フラットのまま維持しており、装飾はアイキャッチの矩形に閉じている。
...
│   └── lib/                      data 読み込み・整形 / art.ts が抽象画の生成
